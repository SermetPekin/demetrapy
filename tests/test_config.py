...
class MethodConfigTest(unittest.TestCase):
    def test_config_alias_is_a_runtime_union(self) -> None:
    ...
    def test_x13_config_normalizes_to_existing_contract(self) -> None:
        ...
        self.assertNotIn("frequency", inspect.signature(X13Config).parameters)
    ...
    def test_tramoseats_config_normalizes_to_existing_contract(self) -> None:
        ...
        self.assertNotIn("frequency", inspect.signature(TramoSeatsConfig).parameters)
        self.assertNotIn("seasonal_filter", inspect.signature(TramoSeatsConfig).parameters)

    def test_typed_config_can_normalize_for_quarterly_data(self) -> None:
        normalized = normalize_config(X13Config(), frequency="Quarterly")

        self.assertEqual(normalized.frequency, "Quarterly")
