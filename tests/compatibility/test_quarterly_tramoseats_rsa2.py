...
FIXTURE_DIRECTORY = (
    Path(__file__).parent / "fixtures" / "quarterly_tramoseats_rsa2"
)
HISTORICAL_COMPONENTS = ("y", "t", "sa", "s", "i", "ycal")
FORECAST_COMPONENTS = ("ycal_f", "s_f", "sa_f", "i_f")


def test_quarterly_rsa2_matches_jdemetra_reference() -> None:
    ...
    result = adjust_dataframe(
        source,
        config=TramoSeatsConfig(spec="RSA2"),
        detailed=True,
    )
    ...
    assert result.for_series("production").specification == "RSA2"